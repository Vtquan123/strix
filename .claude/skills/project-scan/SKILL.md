---
name: project-scan
description: Perform the one-time initial adoption scan of an existing codebase — read the real project (tech stack, structure, conventions, architecture, domain terms) and populate knowledge/* plus an adoption ADR from evidence. Use when Strix is first brought into a real project and knowledge/ is still template placeholders; not for ongoing post-task updates (use knowledge-update for those).
metadata:
  kind: reasoning
  engine: claude
---
...
project and replace the `knowledge/*` template placeholders with evidence-based
...
- The user brings Strix into an existing/real project for the first time and
  `knowledge/*` still holds template placeholders (`_e.g. …_`, generic diagrams).
...
- **In:** read-only access to the target repo — source tree, package manifests
  ...
  configs, CI config, existing READMEs/docs; current `knowledge/*` templates.
- **Out:** populated `project-context.md`, `architecture.md`,
  `coding-conventions.md`, `glossary.md`, plus a new
  `knowledge/decisions/NNNN-adopt-<project>.md` ADR recording the scan.
...
6. **Adoption ADR** — author `knowledge/decisions/NNNN-adopt-<project>.md`
   (per the [decisions README](../../../knowledge/decisions/README.md) numbering):
