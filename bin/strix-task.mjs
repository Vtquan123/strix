 ...
 *
 * Usage:
 *   strix-task new <workstream> --title "..." [--complexity C] [--priority P]
 *   strix-task move <id> <stage>
 *   strix-task where <id>
 *   strix-task ls [--workstream w] [--stage s] [--owner o]
 *   strix-task workstream add <id> --prefix P --owner O
 *   strix-task workstream close <id>
 *   strix-task doctor
 *   strix-task migrate
 *
 * Global: [--board <dir>]  defaults to <cwd>/.strix/tasks
 *
...
function setField(text, name, value) {
  ...
  // No row yet (a pre-migration task has no Workstream): insert after the ID row
  // so the header keeps the order the template generates.
  const idRow = text.match(FIELD_RE('ID'));
  if (!idRow) die(`task file has no header table to write "${name}" into`);
  return text.replace(idRow[0], () => `${idRow[0]}\n| **${name}** | ${value} |`);
}
...
function cmdNew(boardDir, positional, flags) {
  ...
  const complexity = flags.complexity ?? 'STANDARD';
...
function cmdMove(boardDir, positional) {
...
function cmdLs(boardDir, flags) {
  ...
  for (const [ws, items] of [...groups].sort()) {
    ...
    for (const t of items.sort((a, b) => a.id.localeCompare(b.id))) {
      const title = readField(readFileSync(t.path, 'utf8'), 'Title') ?? '';
...
function cmdDoctor(boardDir) {
  ...
  for (const task of scanBoard(boardDir)) {
  ...
  console.log('✓ board valid: status, workstream, prefix, registry');
}
...
function cmdMigrate(boardDir) {
  ...
  for (const task of loose) {
    ...
    renameSync(task.path, dest);
    writeFileSync(dest, setField(readFileSync(dest, 'utf8'), 'Workstream', DEFAULT_WORKSTREAM));
...
const USAGE = `strix-task — manage a Strix task board
  ...
  new <workstream> --title "..." [--complexity C] [--priority P]
  move <id> <stage>            (${STAGES.join(', ')})
  where <id>
...
switch (command) {
  ...
  case 'move': cmdMove(boardDir, positional); break;
