...
## Inputs / Outputs
- **In:** user request, target skill name, the project's existing `.clinerules/skills/*`,
  and the project catalog (see Reference document below).
- **Out:** a created/updated/deleted skill under the project's `.clinerules/skills/<name>/`
  plus a synced project catalog — or an explicit "no change" when the user declines.

## Scope: the project's Cline skills only
This authors implementation skills into the **working project's**
`.clinerules/skills/` (seeded there by `/strix:init`). It never touches the
installed Strix plugin's files (`skills/`, `reference/`, plugin `README.md`,
routing docs) — those are read-only and describe Strix's built-ins, not the
project's skills.

## Reference document to keep in sync
Any create (with rename) or delete of a Cline skill must be reflected in the
**project's own, writable** catalog:

- `.clinerules/skills/README.md` — the project's Implementation Skills list.

Do not edit the plugin's `reference/docs/skills.md`, plugin `README.md`, or the
plugin routing docs; they are read-only in a consuming project.
...
6. Add the skill to the project catalog above (`.clinerules/skills/README.md`).
...
4. If the name changed, rename the folder to match `name` and propagate the
   rename to the project catalog (`.clinerules/skills/README.md`).

**Delete**
1. **Ask** the user to confirm they really want to delete the skill.
2. If **no** → do nothing and stop.
3. If **yes** → remove `.clinerules/skills/<name>/` and its entry from the project
   catalog (`.clinerules/skills/README.md`); verify no project workflow still points to it.
...
- After any create/rename/delete, update the project's `.clinerules/skills/README.md`
  so its list and links stay accurate.
...
- Don't leave a dangling reference: a deleted/renamed skill must vanish from the
  project's `.clinerules/skills/README.md` in the same change.
- Don't edit the installed Strix plugin's files — author only under the project's
  `.clinerules/skills/`.
...
- [ ] Project catalog synced (`.clinerules/skills/README.md`); no plugin files edited
...
`description`, keep the body lean, then add `graphql` to the project's
...
in the project's `.clinerules/skills/README.md`.
